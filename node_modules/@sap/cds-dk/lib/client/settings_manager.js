...
function logConfigPaths() {
    ...
    Logger.debug(`Authentication data is stored in ${CONFIG.paths.auth} (${TOKEN_STORAGE_DESC.plain}) or ${CONFIG.keyringDesignation} (${TOKEN_STORAGE_DESC.keyring})`);
}
...
class SettingsManager {
    ...
    static async loadAndMergeSettings(params, logout = false) {
        ...
        Logger.debug(`Loaded project settings for ${params.get('projectFolder')}: ${loadedSettings.format()}`);
    ...
    static async checkKeytar(params, logout) {
        if (isBas()) {
            if (params.get('tokenStorage') === 'keyring') {
                Logger.log(`NOTE: ${capitalize(TOKEN_STORAGE_DESC.keyring)} not supported on SAP Business Application Studio. Switching to ${TOKEN_STORAGE_DESC.plain}.`);
        ...
        if (params.get('tokenStorage') === 'keyring') {
            throw new Error(`${capitalize(TOKEN_STORAGE_DESC.keyring)} requested but keytar not installed. Run 'npm install -g keytar' or ` +
                `switch to ${TOKEN_STORAGE_DESC.plain} by adding '--plain' (discouraged).`);
        }
        if (params.get('renewLogin') || logout) {
            Logger.log(`Disabling ${TOKEN_STORAGE_DESC.keyring} functionality: 'keytar' module not found. Run 'npm install -g keytar' to install it.`);
        }
    }
    ...
    static async deleteToken(params, fromOtherStorage = false) {
        ...
        if (!fromStorage) {
            Logger.log(`Failed to delete authentication data: none found for ${target}`);
            return;
        }
    ...
    static async _loadAuthFromFile(appUrl, subdomain) {
        ...
        if (!fs.existsSync(CONFIG.paths.auth)) {
            Logger.debug('Authentication-data file not found');
            return emptyData(all);
        }
        ...
        if (!allAuth) {
            Logger.debug('Empty authentication-data file');
            return emptyData(all);
        }
        ...
        if (!allAuth[appUrl]) {
            Logger.debug(`File contains no authentication data for app URL ${appUrl}`);
            return new ParamCollection();
        }
        ...
        if (!authForSubdomain) {
            Logger.debug(`File contains no authentication data for subdomain ${subdomain} (app URL: ${appUrl})`);
            return new ParamCollection();
        }
    ...
    static async _loadAuthFromKeyring(appUrl, subdomain) {
        ...
        if (!authString) {
            Logger.debug(`${capitalize(TOKEN_STORAGE_DESC.keyring)} contains no authentication data for URL ${appUrl} and subdomain '${subdomain}'`);
            return new ParamCollection();
        }
